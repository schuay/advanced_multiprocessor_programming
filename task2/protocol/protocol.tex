...
\verb|g++-4.7| and its default makeflags. The cuckoo lock used the same
compiler as well as makeflags \verb|-Wall -Wextra -pedantic -std=gnu++11 -O3|.

Benchmarks wer assembled by selecting the the minimum execution time of 10 runs
was taken for each value\footnote{With the exception of the \textbf{Striped
...
\textbf{Striped Cuckoo Set} implementation with a 2x1024 locks and our
\textbf{Refined Cuckoo Set}, both with an initial capacity of 2x1024 elements
...
\end{center}
\caption{Performance comparison: \textbf{Striped Cuckoo Set} with different amount of locks}
\label{fig:plot3}
\end{figure}
